...
def generate_report_nessus(path, skip=True, checkAuthOpt=True, internetFacing=False, removeInfo=False, qrtr=False, oldNessus=False):
    ...
    columns_dtypes = {
        ...
        "Risk": pd.CategoricalDtype(categories=severities + [sev.upper() for sev in severities], ordered=True),
        ...
        "See Also": "object",
        "CVE": "object",
        "Plugin ID": pd.Int64Dtype(),
        "Risk Factor": pd.CategoricalDtype(ordered=False),
        "STIG Severity": pd.CategoricalDtype(ordered=False),
        "CVSS v2.0 Base Score": "float64",
        "CVSS v3.0 Base Score": "float64",
        "CVSS v2.0 Temporal Score": "float64",
        "CVSS v3.0 Temporal Score": "float64",
        "BID": "object",
...
def _worksheetFormat(worksheet, writer, df):
    ...
    worksheet.set_column(get_col("Plugin Publication Date"), get_col("Plugin Publication Date"), 21)
...
def stripOutput(df: pd.DataFrame):
    ...
    try:
        df.loc[df["Vulnerability Name"] == "Nessus Scan Information", "Plugin Text"] = df.loc[
            df["Vulnerability Name"] == "Nessus Scan Information", "Plugin Text"
        ].str.replace("Scan policy used : .*", "Scan policy used : NSG Scan", regex=True)
...
def checkPing(df: pd.DataFrame, destpath):
    ...
    try:
        ...
        pass
    pingSuccessText = "The remote host is up"
    printed: bool = False
